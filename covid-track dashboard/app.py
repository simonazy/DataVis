...
'''
 The real-time data from the Center for Systems Science and Engineering (CSSE) 
 at Johns Hopkins University. Refer:https://github.com/CSSEGISandData/COVID-19.
'''
...
# Preprocess the three separate dataset
def data_pivot(file, filename):
    date = file.columns[4:]
    result = file.melt( id_vars=['Province/State', 'Country/Region', 'Lat', 'Long'],
                            value_vars=date,
                            var_name='date',
                            value_name=filename)
    return result 
    
total_confirmed = data_pivot(confirmed, 'confirmed')
total_death = data_pivot(death, 'death')
total_recovered = data_pivot(recovered,'recovered')

# Merge the dataset into one
# Do a left join based on the total_confirmed dataset.
covid_data = total_confirmed.merge(right=total_death, 
                      how='left', 
...
# Change the date to the panads datetime type 
...
app.layout = html.Div([
    ...
    html.Div([
        html.Div([
            html.Img(src=app.get_asset_url('corona-logo.jpeg'),
                     id = 'corona-image',
                     style={'height': '70px',
                            'width': 'auto',
                            'margin-bottom': '25px'})
        ...
        html.Div([
            html.Div([
                ...
                html.H5('Realtime Track of Covid - 19 Cases', style={'margin-bottom': '0px', 'color': 'white'})
            ])
